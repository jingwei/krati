 ...
 * 
 * The key-value pairs are stored in MDSCache using the following data format:
 * <pre>
...
public class SimpleDataStore implements DataStore<byte[], byte[]>
{
    ...
    private long _scn;
    private final MDSCache _cache;
    private final HashFunction<byte[]> _hashFunction;
    
    public SimpleDataStore(MDSCache cache)
    {
        this(cache, new FnvHashFunction());
    }
    
    public SimpleDataStore(MDSCache cache, HashFunction<byte[]> hashFunc)
    {
        this._cache = cache;
