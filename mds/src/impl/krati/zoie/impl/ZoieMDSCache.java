...
public class ZoieMDSCache<R extends IndexReader> implements MDSCache {

	private final ZoieSystem<R,ZoieMDSData> _zoieSystem;
	private final int _idStart;
	private final int _idCount;
	
	public ZoieMDSCache(ZoieSystem<R,ZoieMDSData> zoieSystem,int idStart,int idCount){
	...
	public void deleteData(int memberId, long scn) throws Exception {
		DataEvent<ZoieMDSData> event = new DataEvent<ZoieMDSData>(scn,new ZoieMDSData(memberId));
	...
	private final byte[] fetchData(int memberId) {
		...
		try{
			...
			if (info!=null){
			  ...
			  return doc.getBinaryValue(ZoieMDSInterpreter.STORE_FIELD_NAME);
			}
			else{
			  return null;
			}
	...
	public void setData(int memberId, byte[] data, long scn) throws Exception {
		DataEvent<ZoieMDSData> event = new DataEvent<ZoieMDSData>(scn,new ZoieMDSData(memberId,data));
		_zoieSystem.consume(Arrays.asList(event));
	}

	@Override
	public void setData(int memberId, byte[] data, int offset, int length, long scn) throws Exception {
		DataEvent<ZoieMDSData> event = new DataEvent<ZoieMDSData>(scn,new ZoieMDSData(memberId,data,offset,length));
		_zoieSystem.consume(Arrays.asList(event));
	}
